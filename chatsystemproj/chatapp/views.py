...
from .serializers import UserSerializer, UserListSerializer, ConversationSerializer, MessageSerializer, CreateMessageSerializer
...
class ConvesationListCreateView(generics.ListCreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # FIXED: participants_id → participants__id
        ...
        if existing_conversation.exists():
            return Response(
                {'error': 'A Conversation already exists between these two participants'},
                status=status.HTTP_400_BAD_REQUEST
            )
...
class MessageRetreiveDestroyView(generics.RetrieveDestroyAPIView):
    permission_classes = [IsAuthenticated]
